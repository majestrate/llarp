...
distros = [
    *(("debian", x) for x in ("sid", "stable", "testing", "bullseye", "buster")),
    *(
        ("ubuntu", x)
        for x in ("rolling", "lts", "impish", "hirsute", "focal", "bionic")
    ),
]
...
def print_line(myline, value):
    ...
    if sys.__stdout__.isatty():
        jump = lineno - myline
        print(
            "\033[{jump}A\r\033[K{value}\033[{jump}B\r".format(jump=jump, value=value),
            end="",
        )
        sys.stdout.flush()
    else:
...
def distro_build(distro, arch):
    ...
    if (distro, arch) != (
        ("debian", "stable"),
        "amd64",
    ):  # debian-stable-base/amd64 already built
...
def push_manifest(latest, tags):
    ...
    subprocess.run(
        ["docker", "manifest", "rm", latest], stderr=subprocess.DEVNULL, check=False
    )

...
namespace llarp::sodium
{
  namespace
  {
    auto logcat = log::Cat("crypto");
  }
  static bool
  dh(SharedSecret& out,
  ...
  static bool
  dh_client_priv(
      SharedSecret& shared, const PubKey& pk, const SecretKey& sk, const KeyExchangeNonce& n)
  {
    SharedSecret dh_result{};

    if (not dh(dh_result, sk.toPublic(), pk, pk.data(), sk))
    {
      log::warning(logcat, "dh_client(): failed");
      return false;
    }
    if (crypto_generichash_blake2b(
        ...
        == -1)
    {
      log::warning(logcat, "dh_client(): crypto_generichash_blake2b failed");
      return false;
    }
    return true;
  }
  ...
  {
    SharedSecret dh_result{};
    if (not dh(dh_result, pk, sk.toPublic(), pk.data(), sk))
    {
      log::warning(logcat, "dh_server(): failed");
      return false;
    }
    ...
    if (crypto_generichash_blake2b(
        ...
        == -1)
    {
      log::warning(logcat, "dh_server(): crypto_generichash_blake2b failed");
      return false;
    }
    return true;
  }
  ...
  CryptoLibSodium::dh_client(
      llarp::SharedSecret& shared, const PubKey& pk, const SecretKey& sk, const KeyExchangeNonce& n)
  {
  ...
  {
    const KeyExchangeNonce _n{n.data()};
    return dh_client_priv(shared, pk, sk, _n);
  }
  ...
  {
    const KeyExchangeNonce _n{n.data()};
    return dh_server_priv(shared, pk, sk, _n);
  }

  bool
  CryptoLibSodium::shorthash(ShortHash& result, const llarp_buffer_t& buff)
  {
    return crypto_generichash_blake2b(result.data(), result.size(), buff.base, buff.sz, nullptr, 0)
        != -1;
  }
  ...
  make_scalar(AlignedBuffer<32>& out, const K& k, uint64_t i)
  {
    ...
    ShortHash n;
    return -1
        != crypto_generichash_blake2b(n.data(), SHORTHASHSIZE, buf.data(), buf.size(), nullptr, 0)
...
namespace llarp
{
  ...
  {
    uint64_t i{};
    randombytes(reinterpret_cast<byte_t*>(&i), sizeof(i));
    return i;
  }

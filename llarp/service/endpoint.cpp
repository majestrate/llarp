...
namespace llarp::service
{
  ...
  Endpoint::Tick(llarp_time_t)
  {
    ...
    if (NumInStatus(path::ePathEstablished) > 1)
    {
      ...
      {
        LookupNameAsync(item.first, [name = item.first, info = item.second, this](auto maybe_addr) {
  ...
  Endpoint::HandleGotIntroMessage(dht::GotIntroMessage_constptr msg)
  {
    ...
    if (itr == lookups.end())
    {
      LogWarn("invalid lookup response for hidden service endpoint ", Name(), " txid=", msg->txid);
      return true;
    }
  ...
  Endpoint::GetHopsForBuild()
  {
    ...
    const auto maybe =
        m_router->nodedb()->GetRandom([exclude, r = m_router](const auto& rc) -> bool {
          return exclude.count(rc.pubkey) == 0 and not r->routerProfiling().IsBadForPath(rc.pubkey);
        });
  ...
  struct LookupNameJob : public IServiceLookup
  {
    ...
    {
      auto msg = std::make_shared<routing::DHTMessage>();
      static_assert(dht::Key_t::SIZE == sizeof(namehash));
      msg->M.emplace_back(
          std::make_unique<dht::FindNameMessage>(dht::Key_t{}, dht::Key_t{namehash.data()}, txid));
      return msg;
    }
  ...
  Endpoint::LookupNameAsync(
      std::string name, std::function<void(std::optional<std::variant<Address, RouterID>>)> handler)
  {
      ...
      uint64_t relayOrder)
  {
    ...
    {
      LogError(
          Name(), " failed to lookup ", addr.ToString(), " from ", endpoint, " order=", relayOrder);

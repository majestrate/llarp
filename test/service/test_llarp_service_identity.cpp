...
TEST_CASE("test subkey derivation", "[crypto]")
{
  ...
  const AlignedBuffer<64> root_key_data{
  ...
  const AlignedBuffer<32> root_pub_data{
  ...
  const AlignedBuffer<64> derived_key_data{
  ...
  const AlignedBuffer<32> derived_pub_data{
  ...
  SecretKey root{seed};
  CHECK(root.toPublic().as_array() == root_pub_data);
  ...
  CHECK(Aprime.as_array() == derived_pub_data);
}
...
TEST_CASE("Test sign and encrypt introset", "[crypto]")
{
  ...
  service::Address addr;
  {
    ...
    CHECK(ident.pub.CalculateAddress(a));
    static_assert(decltype(addr)::SIZE == a.size());
    addr = a.data();
  }
  ...
  while(introset.intros.size() < 10)
  {
    ...
    Randomize(intro.router);
    Randomize(intro.pathID);
  ...
  PubKey blind_key;
  const PubKey root_key(addr.data());
  auto crypto = CryptoManager::instance();
